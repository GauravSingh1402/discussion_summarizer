...
const TextForm = ({ onSubmit, onPrev, data }) => {
  ...
  const handleChange = (e) => {
    ...
    const indexes = [];
    let count = 0;
    let currentIndex = e.target.value.indexOf(substring);
    while (currentIndex !== -1) {
      indexes.push(currentIndex);
      currentIndex = e.target.value.indexOf(substring, currentIndex + 1);
    }
    if (indexes.length >= 2) {
      isInterview = true;
    }
...
setValues({ ...values, [e.target.name]: e.target.value,isConversation: isInterview,doCheck:true });
  };
  const handleFile = (e) => {
      ...
      {switchDisplay === "upload" && (
        <>
          ...
          <GrammarlyEditorPlugin
            ...
            clientId="client_Sqej2s7AQxvyGYjq1nqKeW"
          >
            <textarea
              rows={8}
              className="bg-light-primary rounded-md p-5 w-[90%] sm:w-[80%] flex-wrap text-black"
          ...
          <div className="w-full flex flex-row items-center justify-center">
            ...
            <h1 className="font-heading font-semibold text-lg mx-3">OR</h1>
            <hr className="h-[1px] w-[47%] border-0 bg-gradient-to-r from-custom-gradient-start to-custom-gradient-end"></hr>
          </div>
          <label
          ...
          >
            <span className="flex flex-col items-center justify-center space-x-2">
              ...
              <span
                className={`text-sm sm:text-md font-medium text-content text-center`}
              >
              ...
              <div className="mt-1 flex flex-row items-center justify-center px-3 sm:px-5">
                <div className="w-full flex flex-col justify-center">
                  <div className="flex flex-row items-center justify-between">
                    <p className="text-content truncate w-[60%] text-sm sm:text-md">
                      {fileName}
                    </p>
                    <p className="italic text-content text-xs sm:text-sm text-right">
                      {progressText}
                    </p>
                  </div>
                  {uploadProgress > 0 && (
