    ...
    if (email != " ") {
      ...
      const response = await axios
        ...
        .then((response) => {
          ...
          } else if (response.data.data == "User doesnt exist") {
            Swal.fire({
              icon: "error",
              title: "Oops...",
		  ...
		  else if(response.data.data=="Google")
		  {
			Swal.fire({
				icon: "warning",
				title: "This email is already registered using Google",
				text: "You have signed up using Google. Please use Google to login",
			})
		  } 
		  else {
            Swal.fire({
